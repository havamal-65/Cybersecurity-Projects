...
export class SecurityResearcher {
     ...
     */
    async searchThreats(query, timeframe = '7d') {
     ...
     */
    async analyzeIndicators(indicators, depth = 'basic') {
     ...
     */
    async monitorFeeds(keywords, feedTypes = ['all']) {
        ...
        const feedItems = feedResults.results.map((result) => ({
            title: result.title,
            url: result.url,
            source: result.source,
            snippet: result.snippet,
            timestamp: result.timestamp,
            matched_keywords: keywords.filter(keyword => result.title.toLowerCase().includes(keyword.toLowerCase()) ||
     ...
     */
    async researchVulnerabilities(cveIds, severity = 'all') {
        ...
        const vulnerabilities = vulnResults.results.map((result) => ({
            title: result.title,
            url: result.url,
            source: result.source,
            snippet: result.snippet,
     ...
     * Scan web security
     */
    async scanWebSecurity(urls, scanTypes = ['all']) {
    ...
    // Helper methods
    async processThreatResults(results) {
    ...
    detectIndicatorType(indicator) {
        ...
        if (/^[a-fA-F0-9]{32,64}$/.test(indicator))
            return 'hash';
        if (indicator.includes('http://') || indicator.includes('https://'))
            return 'url';
        return 'domain';
    }
    assessReputation(indicator) {
    ...
    calculateOverallThreat(analyses) {
    ...
    analyzeContent(content) {
        if (!content)
            return { secure: false, issues: ['No content'] };
    ...
    calculateSecurityScore(content) {
        ...
        if (content.status === 200)
            score += 10;
        if (content.headers['strict-transport-security'])
            score += 10;
        if (content.headers['content-security-policy'])
            score += 10;
        if (content.headers['x-frame-options'])
            score += 10;
        if (content.headers['x-content-type-options'])
            score += 10;
    ...
    generateScanRecommendations(results) {
        const recommendations = [];
        const successfulScans = results.filter(r => !r.error);
...
//# sourceMappingURL=security-researcher.js.map