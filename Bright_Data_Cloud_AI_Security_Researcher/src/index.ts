...
class BrightDataSecurityMCP {
  ...
  private securityResearcher: SecurityResearcher;
      ...
      try {
        ...
        switch (name) {
          case 'search_security_threats':
            result = await this.securityResearcher.searchThreats(
              (args as any)?.query || '',
              (args as any)?.timeframe || '7d'
            );
            break;

          case 'analyze_threat_indicators':
            result = await this.securityResearcher.analyzeIndicators(
              (args as any)?.indicators || [],
              (args as any)?.analysis_depth || 'basic'
            );
            break;

          case 'monitor_security_feeds':
            result = await this.securityResearcher.monitorFeeds(
              (args as any)?.keywords || [],
              (args as any)?.feed_types || ['all']
            );
            break;

          case 'research_vulnerabilities':
            result = await this.securityResearcher.researchVulnerabilities(
              (args as any)?.cve_ids,
              (args as any)?.severity || 'all'
            );
            break;

          case 'scan_web_security':
            result = await this.securityResearcher.scanWebSecurity(
              (args as any)?.urls || [],
              (args as any)?.scan_types || ['all']
            );
            break;
